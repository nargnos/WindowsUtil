...
namespace NAMESPACE {
	namespace PeDecoderWapper
	{
		...
		public:
			...
			{
				if (SectionHeaderWapper::sortList == nullptr)
				{
					SectionHeaderWapper::sortList = gcnew array<String^>
					{
						"Name",
							"PhysicalAddress",
							"VirtualSize",
				...
				return SectionHeaderWapper::sortList;
			}
			SectionHeaderWapper(IntPtr addr, IntPtr base)
			{
		...
		public:
			...
			SectionHeaderArrayWapper(PeImage^ pe);
			virtual array<String^>^ GetSortList() override
			{
				return nullptr;
			}
