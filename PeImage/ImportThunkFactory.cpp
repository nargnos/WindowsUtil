...
{
	using namespace std;
	ImportThunkIterator ImportThunkFactory::CreateThunkIterator(const IDataDirectoryUtil & util, const PIMAGE_IMPORT_DESCRIPTOR desc)
	{
	...
	ImportThunkIterator ImportThunkFactory::CreateDelayThunkIterator(const IDataDirectoryUtil & util, const PImgDelayDescr descr)
	{
		ImportThunkIterator result(util);
		if (descr != nullptr)
		{
	...
	ImportThunkIterator ImportThunkFactory::CreateThunkIterator(const IDataDirectoryUtil & util, PIMAGE_THUNK_DATA32 thunk, PIMAGE_THUNK_DATA32 originalThunk)
	{
		return ImportThunkIterator(util, thunk, originalThunk);
	}
	ImportThunkIterator ImportThunkFactory::CreateThunkIterator(const IDataDirectoryUtil & util, PIMAGE_THUNK_DATA64 thunk, PIMAGE_THUNK_DATA64 originalThunk)
	{
		return ImportThunkIterator(util, thunk, originalThunk);
	}
	std::unique_ptr<IImportThunk> ImportThunkFactory::CreateThunk(const IDataDirectoryUtil & util, PIMAGE_THUNK_DATA32 thunk, PIMAGE_THUNK_DATA32 originalThunk)
	{
		return make_unique<ImportThunk<PIMAGE_THUNK_DATA32>>(util, thunk, originalThunk);
	}

	std::unique_ptr<IImportThunk> ImportThunkFactory::CreateThunk(const IDataDirectoryUtil & util, PIMAGE_THUNK_DATA64 thunk, PIMAGE_THUNK_DATA64 originalThunk)
	{
