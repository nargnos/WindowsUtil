...
#pragma region Resource
			// TODO:资源目录
			Resource::ResourceDirectoryReader resRoot(pe.GetImageResource());
			Resource::ResourceDirectoryReader tmpRes;
...
#pragma region relocation
			...
			while (brr.Next())
			{
				...
				while (tmpTor.Next())
				{
					DISPLAY_MSG("0x"<<*tmpTor.Current()<<" RVA: "<< tmpTor.CurrentRelocRva()<<" Type: "<<tmpTor.CurrentType());
				}
				TO_LEFT;
				TO_LEFT;
			}
